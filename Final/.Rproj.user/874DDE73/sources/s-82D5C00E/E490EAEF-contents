...
output$single_beg_1_download_data <- renderInfoBox({
  infoBox(
    "Download Data", downloadButton("single_beg_1download", "Download"), icon = icon("database"), color = "navy", fill = TRUE
  )
})
# download handler
...
# TABLE
output$single_beg_1_table <- DT::renderDataTable(single_beg1_df)
